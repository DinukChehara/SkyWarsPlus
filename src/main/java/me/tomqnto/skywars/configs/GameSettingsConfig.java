...
public class GameSettingsConfig {
    ...
    private final static File file = new File(SkywarsPlus.getInstance().getDataFolder(), "game_settings.yml");
    private static FileConfiguration config;

    static {
        if (!file.exists())
            SkywarsPlus.getInstance().saveResource("game_settings.yml", false);
